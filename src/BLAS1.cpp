...
template <typename T> void scal(T alpha, vector<T> &x) {
#pragma omp parallel for
  for (size_t i = 0; i < x.size(); ++i)
    x[i] *= alpha;
}
...
template <typename T> void copy(const vector<T> &x, vector<T> &y) {
  if (x.size() != y.size()) // Check for equal sizes
    throw invalid_argument("Vectors must be of same length!");
...
template <typename T> T nrm1(const vector<T> &x) {
  T sum_abs = T(0);
...
template <typename T> size_t iamax(const vector<T> &x) {
  size_t max_index = 0;
  T max_val = abs(x[0]);
...
template <typename T> size_t iamin(const vector<T> &x) {
  size_t min_index = 0;
  T min_val = abs(x[0]);
