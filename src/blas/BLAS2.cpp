 ...
 * 
 * @param A is m * n Matrix
 * @param B is n * p Matrix
 * @param C is m * p Matrix
 */
template <typename T>
void gemm(T alpha, const vector<vector<T>>& A, const vector<vector<T>>& B, T beta, vector<vector<T>>& C) {
    size_t m = A.size();
    size_t n = A[0].size();
    size_t p = B[0].size();

    if (B.size() != n || C.size() != m || C[0].size() != p)
