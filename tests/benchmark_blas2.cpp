#include "../src/BLAS2.cpp" // Include the implementation file
#include <chrono>
#include <iostream>
#include <vector>
...
void test_performance() {
  ...
  vector<vector<double>> B(n, vector<double>(m, 1.0)); // Matrix B filled with 1.0
  vector<vector<double>> C(m, vector<double>(m, 0.0)); // Matrix C initialized to 0.0
  start = chrono::high_resolution_clock::now();
  gemm<double>(2.0, A, B, 1.0, C); // Explicitly specify template type
  end = chrono::high_resolution_clock::now();
  cout << "GEMM Time: "
       << chrono::duration_cast<chrono::milliseconds>(end - start).count()
       << " ms" << endl;
